...
// Gap (Non-AI): No webhooks for stolen
// No webhooks for stolen-goods database matches
import React, { useState } from 'react';

export default function GapNoWebhooksForStolenPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-stolen/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for stolen</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for stolen-goods database matches</p>
