...
// Gap (AI): Customers without '/customer
// Customers without /customer-lifetime-value (score customer by profitability)
import React, { useState } from 'react';

export default function GapCustomersWithoutCustomerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-customers-without-customer/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Customers without '/customer</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Customers without /customer-lifetime-value (score customer by profitability)</p>
