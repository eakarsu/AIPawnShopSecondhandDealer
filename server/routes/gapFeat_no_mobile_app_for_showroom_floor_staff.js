...
// Gap Feature: No mobile app for showroom floor staff
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-no-mobile-app-for-showroom-floor-staff',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No mobile app for showroom floor staff'.\nProject: AIPawnShopSecondhandDealer.\nContext: No mobile app for showroom floor staff\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-mobile-app-for-showroom-floor-staff', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-mobile-app-for-showroom-floor-staff', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
