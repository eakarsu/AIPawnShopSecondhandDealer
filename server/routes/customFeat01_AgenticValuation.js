...
// Multi-modal analysis (photo, condition notes, comparable sales, market trends, rarity factors); auto-generates appraisal document with confidence intervals
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / cf-agentic-valuation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_valuation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-valuation feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic valuation'.
Context: Multi-modal analysis (photo, condition notes, comparable sales, market trends, rarity factors); auto-generates appraisal document with confidence intervals
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-valuation/run error:', err);
