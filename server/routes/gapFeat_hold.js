...
// Gap Feature: Hold
// Hold-periods without /expiration-optimize (identify items likely to expire unsold; suggest early sale)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-hold',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Hold'.\nProject: AIPawnShopSecondhandDealer.\nContext: Hold-periods without /expiration-optimize (identify items likely to expire unsold; suggest early sale)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['hold', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'hold', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
