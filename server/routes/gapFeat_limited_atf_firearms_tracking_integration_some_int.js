...
// Gap Feature: Limited ATF firearms tracking integration (some integration code exists but no real ATF connector)
// Limited ATF firearms tracking integration (some integration code exists but no real ATF connector)
// Project: AIPawnShopSecondhandDealer
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-limited-atf-firearms-tracking-integration-some-int',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited ATF firearms tracking integration (some integration code exists but no real ATF connector)'.\nProject: AIPawnShopSecondhandDealer.\nContext: Limited ATF firearms tracking integration (some integration code exists but no real ATF connector)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-atf-firearms-tracking-integration-some-int', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-atf-firearms-tracking-integration-some-int', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-atf-firearms-tracking-integration-some-int']
    );
