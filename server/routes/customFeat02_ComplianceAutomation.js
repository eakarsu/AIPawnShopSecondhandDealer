...
// Background process checks new inventory against ATF/NCIC databases; flags stolen items, regulated items missing documentation; auto-quarantines for investigation
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / cf-compliance-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_compliance_automation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-compliance-automation feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Compliance automation'.
Context: Background process checks new inventory against ATF/NCIC databases; flags stolen items, regulated items missing documentation; auto-quarantines for investigation
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-compliance-automation/run error:', err);
