...
// Gap Feature: No webhooks for stolen
// No webhooks for stolen-goods database matches
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-no-webhooks-for-stolen',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No webhooks for stolen'.\nProject: AIPawnShopSecondhandDealer.\nContext: No webhooks for stolen-goods database matches\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-webhooks-for-stolen', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-webhooks-for-stolen', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
