...
// AI identifies high-value customer cohorts; auto-generates personalized offers (e.g., "We got in the guitar you liked..."); SMS/email outreach
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / cf-customer-segmentation-marketing',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_customer_segmentation_marketing`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-customer-segmentation-marketing feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Customer segmentation + marketing'.
Context: AI identifies high-value customer cohorts; auto-generates personalized offers (e.g., "We got in the guitar you liked..."); SMS/email outreach
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-customer-segmentation-marketing/run error:', err);
