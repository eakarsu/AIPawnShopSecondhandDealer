...
// Gap Feature: No integration with NCIC/stolen goods databases (FBI/Interpol)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-no-integration-with-ncic-stolen-goods-databases-fb',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integration with NCIC/stolen goods databases (FBI/Interpol)'.\nProject: AIPawnShopSecondhandDealer.\nContext: No integration with NCIC/stolen goods databases (FBI/Interpol) — only manual police-reports module\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integration-with-ncic-stolen-goods-databases-fb', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integration-with-ncic-stolen-goods-databases-fb', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-integration-with-ncic-stolen-goods-databases-fb']
    );
