...
// Gap Feature: Cash
// Cash-drawer without /theft-detection (flag unusual cash patterns or employee behavior)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-cash',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Cash'.\nProject: AIPawnShopSecondhandDealer.\nContext: Cash-drawer without /theft-detection (flag unusual cash patterns or employee behavior)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['cash', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'cash', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
