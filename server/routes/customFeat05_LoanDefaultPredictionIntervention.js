...
// Monitor loan payment behavior; predict defaults 30-60 days ahead; auto-suggest payment plans or early refinancing
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / cf-loan-default-prediction-intervention',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_loan_default_prediction_intervention`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-loan-default-prediction-intervention feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Loan default prediction + intervention'.
Context: Monitor loan payment behavior; predict defaults 30-60 days ahead; auto-suggest payment plans or early refinancing
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-loan-default-prediction-intervention/run error:', err);
