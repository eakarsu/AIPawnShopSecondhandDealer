...
// Gap Feature: Auctions without \'/auction
// Auctions without /auction-price-suggest (optimize reserve price based on demand prediction)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-auctions-without-auction',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Auctions without \'/auction'.\nProject: AIPawnShopSecondhandDealer.\nContext: Auctions without /auction-price-suggest (optimize reserve price based on demand prediction)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['auctions-without-auction', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'auctions-without-auction', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
