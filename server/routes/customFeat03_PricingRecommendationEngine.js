...
// Real-time market data (eBay, Craigslist, local comps) with ML price elasticity model; suggests pricing to maximize sell-through rate
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / cf-pricing-recommendation-engine',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_pricing_recommendation_engine`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-pricing-recommendation-engine feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Pricing recommendation engine'.
Context: Real-time market data (eBay, Craigslist, local comps) with ML price elasticity model; suggests pricing to maximize sell-through rate
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-pricing-recommendation-engine/run error:', err);
