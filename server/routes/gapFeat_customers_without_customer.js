...
// Gap Feature: Customers without \'/customer
// Customers without /customer-lifetime-value (score customer by profitability)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-customers-without-customer',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Customers without \'/customer'.\nProject: AIPawnShopSecondhandDealer.\nContext: Customers without /customer-lifetime-value (score customer by profitability)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['customers-without-customer', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'customers-without-customer', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
