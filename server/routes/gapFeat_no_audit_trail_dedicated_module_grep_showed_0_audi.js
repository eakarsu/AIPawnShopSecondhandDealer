...
// Gap Feature: No audit trail dedicated module (grep showed 0 audit mentions)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPawnShopSecondhandDealer / gap-no-audit-trail-dedicated-module-grep-showed-0-audi',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No audit trail dedicated module (grep showed 0 audit mentions)'.\nProject: AIPawnShopSecondhandDealer.\nContext: No audit trail dedicated module (grep showed 0 audit mentions)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-audit-trail-dedicated-module-grep-showed-0-audi', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-audit-trail-dedicated-module-grep-showed-0-audi', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-audit-trail-dedicated-module-grep-showed-0-audi']
    );
